...
namespace ide::impl
{
	...
	void user_notifier_base::send_to_console(std::string msg) const
	{
		qDebug(msg.c_str());
	}
	...
	void user_notifier_independ_Impl::notifyEmitWithoutCode(std::string func_name) const
	{
		std::string msg{func_name + " has emitted without code"};
		send_to_console(msg);
	}

	void user_notifier_independ_Impl::notifyInvoke(std::string func_name) const
	{
	...
	void user_notifier_independ_Impl::notifyFunctionSuccess(std::string func_name) const
	{
		std::string msg {func_name + " has been executed properly"};
		send_to_console(msg);
	}

	void user_notifier_independ_Impl::notifyWindowsShown(std::string func_name, std::string window_name) const
	{
